...
import { Bell, Plus, Trash2, Loader2, Check, X, ToggleLeft, ToggleRight } from 'lucide-react'
...
export function NotificationRules({ caseId }: Props) {
  ...
  const recipientsLabel = (r: Rule) => {
    const parts = []
    if (r.notifyAssignee) parts.push('Encargado de tarea')
    ...
    <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
      ...
      {loading ? (
        ...
        <p className="px-5 py-8 text-sm text-slate-400 text-center italic">
          Sin reglas configuradas — las tareas de este proceso no enviarán alertas de plazo.
        </p>
      ) : (
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-50">
            {rules.map(rule => (
              <tr key={rule.id} className={`hover:bg-slate-50 transition-colors ${!rule.isActive ? 'opacity-50' : ''}`}>
                ...
                {isAdmin && (
                  <td className="px-4 py-3">
                    <button onClick={() => handleDelete(rule.id)} disabled={deleting === rule.id}
                      className="p-1.5 rounded-lg text-slate-400 hover:text-red-500 hover:bg-red-50 transition-colors disabled:opacity-50 mx-auto flex">
