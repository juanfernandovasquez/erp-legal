...
export const DropdownMenuItem = React.forwardRef<HTMLButtonElement, DropdownMenuItemProps>(
  ({ className, ...props }, ref) => (
    <button
      ref={ref}
      className={cn(
        'w-full px-4 py-2 text-left text-sm text-slate-700 hover:text-primary-700 hover:bg-slate-50 transition-all duration-[250ms] ease',
...
export const DropdownMenuCheckboxItem = React.forwardRef<HTMLInputElement, DropdownMenuCheckboxItemProps>(
  ({ className, label, checked, onChange, ...props }, ref) => (
    <label className="flex items-center px-4 py-2 text-sm text-slate-700 hover:text-primary-700 hover:bg-slate-50 transition-all duration-[250ms] ease cursor-pointer border-b border-slate-100 last:border-0">
      <input
        ref={ref}
        type="checkbox"
        checked={checked}
